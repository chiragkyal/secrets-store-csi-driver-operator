...
func RunOperator(ctx context.Context, controllerConfig *controllercmd.ControllerContext) error {
	...
	// clusterCSIDriverLister reads driverConfig.secretsStore configuration off
	// the same ClusterCSIDriver singleton the GenericOperatorClient above
	// already watches. dynamicInformers.ForResource(gvr) returns that exact
	// same shared informer/cache (SharedInformerFactory keys informers by
	// GVR), so wrapping its Lister() gets typed *opv1.ClusterCSIDriver
	// access -- the same call-site ergonomics a dedicated operatorv1informers
	// watch would give -- without opening a second, independent watch on the
	// identical object.
	clusterCSIDriverLister := newTypedClusterCSIDriverLister(dynamicInformers.ForResource(gvr).Lister())
