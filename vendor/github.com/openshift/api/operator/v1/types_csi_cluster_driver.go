...
type CustomSecretRotation struct {
	// minimumRefreshAge is the minimum time in seconds between secret rotation
	// attempts. Each time kubelet calls NodePublishVolume, the driver checks
	// whether this interval has elapsed since the last successful provider call.
	// If it has, the driver contacts the secret provider to fetch the latest
	// secret values and updates the mounted volume.
	// Setting this value below the kubelet syncFrequency (default: 1 minute,
	// configurable via KubeletConfig) has no additional effect on the actual
	// rotation cadence, as NodePublishVolume calls are gated by that frequency.
