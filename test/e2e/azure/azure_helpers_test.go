...
// only for the oc/helm CLIs -- all Azure resource CRUD below goes through
...
// azSubscriptionID and azTenant are captured once in azInit and reused by
// every SDK client/call below.
var (
...
// azKeyVaultSetPolicy grants objectID "get" secret permission on vaultName.
...
func azKeyVaultDelete(name, resourceGroup string) error {
	if _, err := vaultsClient.Delete(context.Background(), resourceGroup, name, nil); err != nil {
		return fmt.Errorf("unable to delete Key Vault %q: %w", name, err)
	}
	_, err := vaultsClient.BeginPurgeDeleted(context.Background(), name, location, nil)
	return err
}
